...
- `CODE_HASH_SECRET`: secret สำหรับ HMAC รหัส 6 หลัก; ถ้าไม่ตั้งจะใช้ `JWT_SECRET`
- `JWT_EXPIRES_IN`: ค่าเริ่มต้น `7d`
- `CORS_ORIGIN`: origin ของ frontend
- `TRUST_PROXY_HOPS`: จำนวน reverse proxy ที่เชื่อถือได้ (`0` สำหรับ local)
...
1. **ยืนยันอีเมล** — ระบบส่ง**รหัส 6 หลัก**พร้อมลิงก์สำรองไปทางอีเมล ผู้สมัครกรอกรหัสในหน้าสมัครได้เลย
   ...
   เก็บโทเคนแบบ hash และเก็บรหัส 6 หลักแบบ HMAC ด้วย secret ฝั่งเซิร์ฟเวอร์ · กรอกผิดเกิน 5 ครั้งต้องขอรหัสใหม่ · ขอรหัสใหม่ = รหัสเก่าใช้ไม่ได้ทันที
...
ทุกการเปลี่ยนสถานะตรวจจากแพ็กเกจ `@udomsasn/workflow` และเขียน `StatusLog` ใน transaction เดียวกันกับการเปลี่ยนข้อมูล ผลตรวจผูกกับ `mediaVersion` และผล AI เก็บใน `AiReview` เท่านั้น
